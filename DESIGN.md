...
| Session title header (M34) | the session's name at the TOP of the chat, as a sibling of the transcript (the transcript is the scroller, so the name stays while messages move) — `--fs-lg`/600, deliberately one step under the page-level `--fs-xxl` header. Rename is in place (a `.field` at `--fs-md` + Save/Cancel, Escape cancels) and **Suggest title** proposes a name; a proposal is a well (`--surface-2`, no border, no elevation) with Use/Dismiss, and when no model named it the note says so. Nothing here is stored until the owner accepts. |
| Sidebar nav | active item: accent text on surface-2 pill; icons 16–20px, no glow. Collapses to the 60px icon rail (one `--side-w` knob, `.app.side-minimized`); icon-only items carry the label as a `title`, and the badge sits in the button's corner rather than being hidden. Groups are **Workspace / Studio / Tools / Settings** (M31): Studio is what the partner is made of (personas, skills, playbooks), Tools is its working data (files, memory), and every configuration surface (providers, themes, audit, members) lives under the last group. The session tree nests under **Personas** (M32), and the menu's right edge is a drag divider. The folder half of a session's organization is its own destination — **Workspace / Chat · Folders · Notes · Shared** (M34). |
...
| Folders page (M34, Explorer in M35) | the folder half of a session's organization, as an **Explorer**: kicker → `--fs-xxl` title → copy → an address row (`↑ Up`, a breadcrumb whose crumbs are controls with the current one `aria-current="page"`, the item count, one Create action) over a two-pane body — a **navigation pane** (the embedded conversation rail in its folders-only `nav` mode: selectable folder rows, current row = `--surface-2` fill + weight, never a border) beside a **contents pane** (`FolderContents`: the open folder's subfolders then chats in a shared grid of Name · Type · Items · **Assets** · Modified; rows separate by hover/selection fill, not rules). Assets is per-chat (the assets saved in that session, M11 F10) and summed over a folder's subtree, so a parent row cannot read 0 while its subfolder holds some; a chat with none reads "0 assets" — a column is a measurement, and 0 is the measurement. Page scrolls; 1200px measure (the system's widest — dense tool surface, not prose); neither pane carries a card (space → background shift → elevation). A folder row's controls are the shared `FolderActions` (rail and contents pane render it identically), a chat row's are `ChatActions`. Phone tier: panes stack, Type/Items/Assets/Modified drop first, item count yields so the breadcrumb is not ellipsised. Persona grouping stays under **Personas**; the two axes never write each other. |
