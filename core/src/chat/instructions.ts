...
export function authoringInstructions(
  ...
  return [
    ...
    '  3. To CHANGE a skill the user already has installed, pass its id as',
    '     "skillId" to the drafting tool: the bundle is staged as an UPDATE to',
    '     that skill (same ask, same approval). If the update would WIDEN what the',
    '     skill may do, the result lists the change table and the user has to',
    '     acknowledge it before approving — say what it adds when you ask. Nothing',
    '     changes until they approve.',
    '  4. You cannot run a draft either. Only the user test-runs it, in the Studio',
